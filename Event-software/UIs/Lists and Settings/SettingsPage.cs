...
namespace itHappens.UIs.valentina
{
    public partial class SettingsPage : UserControl
    {
        ...
        public SettingsPage()
        {
            ...
            {
                UsersID = Classes.DatabaseGeneralMethods.returnUsersIDWhenIsLogedIn(UIs.Sidebars.ProfileSidebar.Instance.usernameLable.Text);
        ...
        {
            UsernameTextBox.Text = UIs.Sidebars.ProfileSidebar.Instance.usernameLable.Text;
            MySqlConnection con;
            int areaID=0;
            ...
            try
            {
                ...
                String queryString = "Select name,surname,email,areaID from users where username= '" + 
                    UIs.Sidebars.ProfileSidebar.Instance.usernameLable.Text + "'";
        ...
        private void UsernameTextBox_Validated(object sender, EventArgs e)
        {
            if (!UsernameTextBox.Text.Equals(""))
            {
                if (Char.IsLetter(UsernameTextBox.Text[0]))
                {
                    if (UIs.anna.LogInPage.loggedInUser == true)
                    {
                        int userID = Classes.DatabaseGeneralMethods.returnUsersIDWhenIsLogedIn(UIs.Sidebars.ProfileSidebar.Instance.usernameLable.Text);
                 ...
                 { 
                    ...
                    else
                    {
                        if (checkIfThereAreNotChangesInFields(NameSettingsTextBox.Text, SurnameSettingsTextBox.Text, UsernameTextBox.Text, EmailSettingsTextBox.Text))
                        {
                            ...
                            if (!UIs.Sidebars.ProfileSidebar.Instance.usernameLable.Text.Equals(UsernameTextBox.Text))
                            {
                                UIs.Sidebars.ProfileSidebar.Instance.usernameLable.Text = UsernameTextBox.Text;
                            }
                        }
                        else
                        {
